...
namespace BackpropagationAndPerceptron
{
    public partial class SimulacionInterna : Form
    {
                     ...
                     int capasocultas, DataGridView grilladecapasocultas, string funcionsalida)
        {
            ...
            for (int i = 0; i < grilladesimulacion.Rows.Count; i++)
            {
        ...
        private void grilladesimulacion_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (grilladesimulacion.Columns[e.ColumnIndex].Name.Equals("btnAccion"))
            {
                for (int i = 0; i < grilladesimulacion.Rows.Count; i++)
                {
                    if (e.RowIndex == i)
        ...
        private void Simular(int r)
        {
            ...
            {
                grilladesimulacion.Rows[r].Cells[i].Value = arraydesalidadelacapaactual[pt, i2].ToString();
                i2++;
            }
        }
